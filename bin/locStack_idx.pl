...
print '                                                                  
           _             _____ _             _      _     _      
          | |           / ____| |           | |    (_)   | |     
          | | ___   ___| (___ | |_ __ _  ___| | __  _  __| |_  __
          | |/ _ \ / __|\___ \| __/ _` |/ __| |/ / | |/ _` \ \/ /
          | | (_) | (__ ____) | || (_| | (__|   <  | | (_| |>  < 
          |_|\___/ \___|_____/ \__\__,_|\___|_|\_\ |_|\__,_/_/\_\
            Meta analysis of genomic features  ______  indexed           
                                              |______|           
...
#($alignedreadhash,$totalExpReads)=ChattaIdx::READALIGNEDTAGS($exp,$extend_length,$rscore,$dcol);

print "     $totalExpReads tags processed from $exp file\n\n";

$time_tag=Time::PRINTTIME("Loaded aligned reads ",$start_time,$time_tag);

($summitsFwd,$summitsRev,$totalfwdsummits,$totalrevsummits)=ChattaIdx::READSUMMITS($summitfile);

$time_tag=Time::PRINTTIME("Loaded reference coordinates ",$start_time,$time_tag);

@bincontainer = ChattaIdx::BINS($binwidth,$bflank,$aflank);
...
my $FwdDist =ChattaIdx::RETURNBINCOUNTSALLCHR ($summitsFwd,$exp,$control,\@bincontainer,$binwidth,$extend_length,$normalize,$totalExpReads,$totalExpReads,$nproc,$genome,$rscore,$dcol);
...
my $RevDist =ChattaIdx::RETURNBINCOUNTSALLCHR ($summitsRev,$exp,$control,\@bincontainer,$binwidth,$extend_length,$normalize,$totalExpReads,$totalExpReads,$nproc,$genome,$rscore,$dcol);
